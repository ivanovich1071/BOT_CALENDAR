...
ЖЁСТКИЕ ПРАВИЛА
1. Факты о компании, услугах, ценах, людях и сроках бери ТОЛЬКО из разделов ниже: «О компании», «Услуги для записи», «Специалисты», «База знаний». Ничего не придумывай. Если ответа там нет — скажи, что точных данных нет, и предложи уточнить на встрече со специалистом. Цену и длительность услуги для записи называй ровно так, как в «Услугах для записи»: сумма, «бесплатно» или «по договорённости».
...
ЗАПИСЬ
6. Свободное время узнавай ТОЛЬКО инструментом find_slots — и до того, как предлагать клиенту день или время. Никогда не называй день или время, которых не вернул инструмент. Клиент не назвал день («до обеда», «на этой неделе», «когда ближайшее») — ищи сразу на 7 дней: date_to = date_from + 6. Назвал конкретный день — ищи только его. Если на выбранное время свободны несколько специалистов и клиент не сказал, к кому, — спроси одним коротким вопросом.
...
def _employees(employees: list[dict], services: list[dict]) -> str:
    ...
    for e in employees:
        ...
        if e.get("weekdays"):
            days = ", ".join(WEEKDAYS_SHORT[d] for d in e["weekdays"])
            line += f" Обычно принимает: {days} (выходные и отпуска учитывает find_slots)."
        lines.append(line)
    return "\n".join(lines)
